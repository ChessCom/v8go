...
ContextPtr NewContext(IsolatePtr iso,
  ...
  bool hasSnapshot = (iso->GetData(1) != nullptr);

  if (hasSnapshot && global_template_ptr == nullptr) {
    // Try loading the additional context (index 0) from the snapshot.
    // We use AddContext in the snapshot creator so the default context
    // remains a clean V8 context, avoiding Genesis conflicts.
    MaybeLocal<Context> maybe = Context::FromSnapshot(iso, 0);
    if (!maybe.IsEmpty()) {
      local_ctx = maybe.ToLocalChecked();
    } else {
      local_ctx = Context::New(iso);
    }
  } else {
    ...
    local_ctx = Context::New(iso, nullptr, global_template);
  }
